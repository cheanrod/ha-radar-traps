...
# The original scraper polled every ten minutes; entries live for hours, so
# there is nothing to gain from hammering the page harder.
DEFAULT_SCAN_INTERVAL_MINUTES: Final = 10
MIN_SCAN_INTERVAL_MINUTES: Final = 5
MAX_SCAN_INTERVAL_MINUTES: Final = 180

# What the predecessor service returned when the list was empty. Kept so the
# `messages` attribute is a drop-in replacement for its JSON payload.
